...
<head>
    ...
    <title>Sorties de la Catégorie — Club d'Escalade</title>
...
<body class="min-h-screen bg-zinc-950 text-zinc-100 antialiased pb-16">
    ...
    <main class="mx-auto max-w-4xl px-6 py-12">
        ...
        <!-- Header -->
        <header class="mb-8 rounded-xl border border-zinc-800 bg-zinc-900/30 p-6 shadow-md relative">
            <h1 class="text-2xl font-extrabold text-white leading-tight">
                Discipline : <span class="text-emerald-500"><c:out value="${categorie.categorieName}" /></span>
            </h1>
            <div class="mt-4 flex flex-wrap gap-2">
                <a href="<c:url value='/categories' />" class="inline-flex items-center justify-center gap-1.5 rounded-lg border border-zinc-800 bg-zinc-900/60 px-3.5 py-2 text-xs font-bold text-zinc-300 hover:bg-zinc-800 hover:text-white transition duration-150">
                    ← Catégories
                </a>
        ...
        <c:if test="${empty sorties}">
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/30 p-12 text-center text-zinc-500 shadow-md">
                ...
                Aucune aventure de type "<c:out value='${categorie.categorieName}' />" planifiée pour le moment.
            </div>
        </c:if>
